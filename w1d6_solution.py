...
if sys.platform == "win32":
    CL_EXE_FOLDER = "C:/Program Files (x86)/Microsoft Visual Studio/2017/Community/VC/Tools/MSVC/14.16.27023/bin/Hostx64/x64"
...
# %%
if "SOLUTION":
    filename = "w1d6_kernels/abc_soln.cu"
else:
...
# %%
if "SOLUTION":
    filename = "w1d6_kernels/sum_atomic_soln.cu"
else:
...
def sum_atomic(inp: t.Tensor, dest: t.Tensor, block_size: int = 1024) -> None:
    ...
    "SOLUTION"
    nelem = np.product(inp.shape, dtype=np.int64)
    n_blocks = int(ceil_divide(nelem, block_size))
    sum_atomic_kernel(
        Holder(inp),
        Holder(dest),
        nelem,
...
# %%
if "SOLUTION":
    filename = "w1d6_kernels/filter_atomic_soln.cu"
else:
...
# %%
if "SOLUTION":
    filename = "w1d6_kernels/index_soln.cu"
else:
...
# %%
if "SOLUTION":
    filename = "w1d6_kernels/sum_rows_soln.cu"
else:
...
"""
# %%
inp1 = t.randn((10_000_000, 10), device=device)
inp2 = t.randn((10, 10_000_000), device=device)
...
# %%
if "SOLUTION":
    filename = "w1d6_kernels/sum_fast_soln.cu"
else:
...
def sum_n_blocks(inp: t.Tensor, dest: t.Tensor, block_size=1024):
    ...
    """SOLUTION"""
    n = inp.numel()
    n_blocks = ceil_divide(n, block_size)
    print(f"Launching {n_blocks} blocks")
    sum_1024_kernel(
        Holder(inp),
        Holder(dest),
        np.int32(n),
