...
def stable_diffusion_inference(
    ...
    with t.autocast("cuda"):
        for i, ts in enumerate(scheduler.timesteps):
            ...
            latent_input = None
            if "SOLUTION":
                latent_input = latents.repeat((2, 1, 1, 1)) / (scheduler.sigmas[i] ** 2 + 1).sqrt()
            # Predict noise in latent input
            with t.no_grad():
                if "SOLUTION":
                    noise_pred_concat = pretrained.unet(latent_input, ts, encoder_hidden_states=text_embeddings)[
                        "sample"
                    ]
            if "SOLUTION":
...
if MAIN:
    SEED = 1
    config = StableDiffusionConfig(t.manual_seed(SEED))  # Pass in seed generator to create the initial latent noise
...
def latent_sample_same(config: StableDiffusionConfig, batch_size: int) -> t.Tensor:
    if "SOLUTION":
        latents = t.randn(
            (1, cast(int, pretrained.unet.in_channels), config.height // 8, config.width // 8),
            generator=config.generator,
        ).to(DEVICE)
