...
"""
# TBD: pytorch allows backward only on scalars; I don't think this behavior is actually that important to copy for the conceptual understanding here. It's more of a "prevent user error" thing, yeah?
# TBD lowpri: use lowercase tensor function instead of Tensor to generate tensors. Explain the difference if we didn't already.
# TBD lowpri: mess with typecheck, we don't gain much but particularly methods probably should coerce scalars to tensors
# %%
import os
import time
...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    """
    "SOLUTION"
    # Sum and discard leading dimensions that were added
    n_leading = broadcasted.ndim - original.ndim
    if n_leading > 0:
        broadcasted = broadcasted.sum(axis=tuple(range(n_leading)))
    assert broadcasted.ndim == original.ndim
    # Sum and keep 1-size dimensions that were grown
    grown_dims = tuple([axis for axis, size in enumerate(original.shape) if size == 1 and broadcasted.shape[axis] > 1])
    broadcasted = broadcasted.sum(axis=grown_dims, keepdims=True)
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        "SOLUTION"
        args_unboxed = tuple([arg.array if isinstance(arg, Tensor) else arg for arg in args])
        for k, v in kwargs.items():
            if isinstance(v, Tensor):
                raise ValueError(f"Keyword tensors not supported, got key: {k}, value: {v}")
        out = Tensor(numpy_func(*args_unboxed, **kwargs))
        if grad_tracking_enabled and is_differentiable:
            ...
            if parents:
                out.recipe = Recipe(numpy_func, args_unboxed, kwargs, parents)
                out.requires_grad = True
        return out
...
def _sum(x: Arr, dim=None, keepdim=False) -> Arr:
    ...
    "SOLUTION"
    return np.sum(x, axis=_coerce_dim(x, dim), keepdims=keepdim)
...
def _getitem(x: Arr, index: Index) -> Arr:
    ...
    if isinstance(index, int):
        ...
        unboxed = tuple([_coerce_index(item) for item in index])
        return x[unboxed]
...
def _matmul2d(x: Arr, y: Arr) -> Arr:
    ...
    assert x1 == y0
    out = np.matmul(x, y)
    assert out.shape == (x0, y1)
    return out


def matmul2d_back0(grad_out: Arr, out: Arr, x: Arr, y: Arr) -> Arr:
    "SOLUTION"
    # Note: no broadcasting possible in the simple version
    # Full version does broadcast over batch dimensions
    return np.matmul(grad_out, np.transpose(y))


def matmul2d_back1(grad_out: Arr, out: Arr, x: Arr, y: Arr) -> Arr:
    "SOLUTION"
    return np.matmul(np.transpose(x), grad_out)
...
"""
# TBD: explain why Parameter should be a Tensor subclass - this could be quite surprising
...
"""
# TBD: I don't think we actually need the getattr bypass - check this
...
def visualize(dataloader):
    ...
    for i in range(10):
        ...
        plt.xticks([])
        plt.yticks([])
        plt.imshow(sample[i, 0], cmap=plt.cm.binary)  # type: ignore
    plt.show()
...
def test(model, test_loader):
    ...
    with NoGrad():
        for data, target in test_loader:
            ...
            target = Tensor(target.numpy())
            output = model(data)
            test_loss += cross_entropy(output, target).sum().item()  # sum up batch loss
